...
function loadDirectory (dir, widgets, pathArray, sandal, callback) {
	...
	async.each(fs.readdirSync(dir), function (file, fileCallback) {
		...
		try {
			...
			return fileCallback(new Error('Failed loading file ' + widgetPath + ': ' + e.message));
		}
		...
		resolveWidget(sandal, code, function (err, resolvedCode) {
			...
			var widget = {
				path: pathArrayClone,
				type: pathArrayClone.join('.'),
				dir: widgetDir,
				options: resolvedCode.options,
				template: hasDefaultTemplate ? templatePath : null,
				less: (!fs.existsSync(lessPath) || !fs.statSync(lessPath).isFile()) ? null : lessPath,
				cssClass: 'widget-' + pathArrayClone.join('_'),
				themes: themes,
				scripts: glob.sync(scriptPath),
				factory: resolvedCode.factory || function (params, callback) { callback(null, params || {}); },
				test: resolvedCode.test || function (data, callback) { callback(null, true); }
			};
